              ...
              {usersList.map((item) => (
                <div key={item.id} className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h3 className="font-bold text-slate-900 text-sm">{item.name || "Anonymous Partner"}</h3>
                      <p className="text-xs text-slate-500 mt-0.5">{item.email || "No Email Registered"}</p>
                    </div>
                    <span
                  ...
                  <p className="text-[10px] font-mono text-slate-400 mb-3 truncate">ID: {item.id}</p>
